    ...
    <section class="products" id="placeholder">
        ...
        <div class="sort-container">
            ...
            <button class="sort-button <?= $database->namesort ? 'active-sort' : '' ?>" type="button"
                onclick="window.location.href='?shop_ID=<?=$_GET['shop_ID']?>&sort=<?= $database->sort1 ? 'name-asc' : 'name-desc'; ?><?php if (isset($_GET['search'])) {
            ...
            <button class="sort-button <?= $database->pricesort ? 'active-sort' : '' ?>" type="button"
                onclick="window.location.href='?shop_ID=<?=$_GET['shop_ID']?>&sort=<?= $database->sort2 ? 'price-asc' : 'price-desc'; ?><?php if (isset($_GET['search'])) {
                          echo '&search=' . $_GET['search'];
                      } ?>'">Price
                <?= $database->sort2 ? '<i class="bi bi-arrow-up up"></i>' : '<i class="bi bi-arrow-down down"></i>' ?></button>
            <?php if (isset($_GET['sort'])): ?>
                <button class="sort-button reset-sort" type="button" onclick="window.location.href='?shop_ID=<?=$_GET['shop_ID']?><?php if (isset($_GET['search'])) {echo '&search=' . $_GET['search']; }?>'">Reset</button>
            <?php endif; ?>
        </div>
